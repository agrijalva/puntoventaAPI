...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `puntoventa`.`inventario` ;

CREATE TABLE IF NOT EXISTS `puntoventa`.`inventario` (
  `idInventario` INT NOT NULL AUTO_INCREMENT,
  `inv_nombre` VARCHAR(500) NULL,
  `inv_descripcion` VARCHAR(1000) NULL,
  `timestamp` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `idSucursal` INT(11) NOT NULL,
  `idEmpresa` INT(11) NOT NULL,
  `idUsuario` INT(11) NOT NULL,
  `inv_estatus` INT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `puntoventa`.`stockhistorial` (
  ...
  CONSTRAINT `fk_stockhistorial_inventario1`
    ...
    REFERENCES `puntoventa`.`inventario` (`idInventario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;
